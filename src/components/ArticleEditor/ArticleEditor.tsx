...
export default function ArticleEditor({
	...
	return (
		<div>
			<ul className='flex flex-row justify-center m-1.5'>
				{
					tabs.map((tab, index) => {
						return (
							<li
								key={index}
								className={`inline-block mr-2 border border-black dark:border-white dark:text-white p-2 rounded cursor-pointer ${currentTab === index ? 'bg-slate-400 dark:bg-slate-800' : 'bg-slate-200 dark:bg-slate-600'}`}
								onClick={() => {
									setCurrentTab(index)
								}}
							>
